...
vec4 effect(vec4 fragColor, Image texture, vec2 fragCoord, vec2 screenPos)
{
	...
	fragColor.r = texture2D(texture, uv_r).r;
	fragColor.g = texture2D(texture, uv_g).g;
	fragColor.b = texture2D(texture, uv_b).b;
	fragColor.a = texture2D(texture, fragCoord).a;
